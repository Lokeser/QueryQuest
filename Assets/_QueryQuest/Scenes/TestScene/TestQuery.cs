...
public class TestQuery : MonoBehaviour
{
    void Start()
    {
        ...
        var r1 = interpreter.Execute("SELECT * FROM Magias");
        Debug.Log(r1.FormattedLog);

        // Teste 2: Filtro por elemento
        var r2 = interpreter.Execute("SELECT * FROM Magias WHERE Elemento = 'Fogo'");
        Debug.Log(r2.FormattedLog);

        // Teste 3: AND composto
        var r3 = interpreter.Execute("SELECT * FROM Magias WHERE Elemento = 'Agua' AND Distancia = 'LONGO'");
        Debug.Log(r3.FormattedLog);

        // Teste 4: Erro de coluna
        var r4 = interpreter.Execute("SELECT * FROM Magias WHERE Distorcia = 'Fogo'");
